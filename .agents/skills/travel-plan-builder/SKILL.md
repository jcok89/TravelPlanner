...
구조·데이터 모델은 아래 명세를 그대로 따른다. **레퍼런스 구현: 이 저장소의 `예시_202610_오사카교토/index.html`** (배포 키트 검증용 가상 테스트 케이스 — 이 파일의 구조를 그대로 따르면 된다).
...
- **날짜별 예약가능 확인된 마이리얼트립 투어는 해당 날짜의 stop 또는 alts 카드로 임베드**한다(상품명·평점·시작가·예약 URL·`getTnaOptions`로 확인한 날짜 표기). 동선/시간이 겹치거나 예약 불가면 계획서에 넣지 말고 그 사실을 research에 남긴다.
- 해외 여행은 **가격에 현지통화+원화를 병기**하고, 상단이나 해당 항목에 **환율 가정(예: 100엔≈930원)과 "결제화면 기준"** 을 명시한다.
